...
export default function LaunchpadPage() {
  ...
  const analyze = async () => {
    const payload = inputType === 'url' ? { url: value } : inputType === 'pitch' ? { pitchDeck: value } : { description: value };
        ...
        <div className="flex gap-4">
          {['description', 'url', 'pitch'].map(t => (
            <button
              ...
              className={`px-4 py-2 rounded ${inputType === t ? 'bg-aura-teal text-white' : 'bg-gray-700'}`}
            >
        ...
        <div className="glass-card space-y-4">
          ...
          {result.expert_notes && (
            <div className="border-t border-white/10 pt-3">
              <p className="text-sm text-gray-400">Expert Review:</p>
