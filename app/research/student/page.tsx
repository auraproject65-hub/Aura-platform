...
export default function StudentResearch() {
  ...
  const runResearch = async () => {
    const res = await fetch('/api/research/student', {
      method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-serif">Student Research Workspace</h2>
      <div className="glass-card flex gap-2">
        <input
          className="flex-1 bg-transparent border border-gray-600 rounded-lg p-3"
          value={query}
          onChange={e => setQuery(e.target.value)}
          placeholder="e.g., Review the literature on AI in education"
        />
        ...
        <div className="glass-card space-y-4">
          {report.sections.map((sec: any, i: number) => (
            <div key={i}>
              ...
              {report.sources && i === report.sections.length-1 && (
                <div className="mt-2 text-sm text-gray-400">
                  <strong>References:</strong>
                  <ul className="list-disc list-inside">
                    {report.sources.map((s: string, idx: number) => <li key={idx}>{s}</li>)}
                  </ul>
                </div>
              )}
            </div>
